...
for i = 1:m
  ...
  %fprintf('size of a3i %d %d\n',size(a3i));
  temp = sum(yvec .* log(a3) + (1-yvec) .* log(1-a3));
  %a3i = a3(i,:)';
  %temp = sum(yvec .* log(a3i) + (1-yvec) .* log(1-a3i));
  %fprintf('size of temp %d %d\n',size(temp));
  J = J + temp;
  ...
  z21 = [1;z2];
  delta2 =Theta2' * delta3 .* sigmoidGradient(z21);
  delta2 = delta2(2:end);
